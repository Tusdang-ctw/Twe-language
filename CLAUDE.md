...
**v0.2 work also runs in parallel** — Phase 8 in the roadmap. **Phase 8 is now substantively complete** as of 2026-04-30. Nine feature sessions shipped: 1 (`.glb`), 2a / 2b / 2c (resumable wait + frame stack + VM nested-block parity), 3 (mouse input), 4 (save / load bottom layer), 5 (audio v2), 6 (tilemap), 7 (VM function-body wait via multi-frame fiber save). The remaining line item — **NaN-tagged 64-bit values + tracing GC** — broke out as Phase 8.5 per `docs/08-nan-tagging.md` because the migration is genuinely 9 sub-sessions. **Sessions 8a–8f shipped** (8a–8e on 2026-04-30, 8f finished 2026-05-01 across three commits: structural half, predicate-dispatch pass, and shim deletion). The legacy `Value` enum, `to_legacy` / `from_legacy` shim, and `ToLegacyShim` trait are all deleted. **Sessions 8g–8i still pending** (GC heap allocator; roots wiring; bench + tune to 3× exit). **496 tests pass; clippy + build clean.**
