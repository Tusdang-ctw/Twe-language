...
- **Value representation: NaN-tagged 64-bit values** (in the bytecode VM). Follow *Crafting Interpreters* Chapter 30. **Lands in Phase 8.5** per `docs/08-nan-tagging.md` — its own multi-session sub-phase pulled out of Phase 8's close-out because the migration is genuinely 9 sessions of careful work. Sessions 8a–8e shipped 2026-04-30: 8a (TaggedValue module), 8b (HeapBody expansion), 8c (VM stack + globals on TaggedValue, full-Value shim), 8d (Env::bindings + Instance::fields on TaggedValue), 8e (Object::fields + BcInstance::fields on TaggedValue, stdlib bootstrap + save shimmed). **Session 8f structural half shipped 2026-04-30**: `Value` is now a type alias for `TaggedValue` (the legacy enum was renamed to `LegacyValue`); every storage location (`field_defaults`, `Env::bindings`, `BuiltinFn` signature, `HeapBody::Tuple` / `List` interiors, etc.) is `TaggedValue`; ~550 of 917 `Value::Foo(…)` constructor / storage sites mass-rewrote to `Value::from_*` API. The remaining ~370 sites use `match X.to_legacy() { LegacyValue::Foo(x) => … }` as a transitional shim — that predicate-dispatch cleanup is the 8f-followup session before `LegacyValue` + `to_legacy` / `from_legacy` delete. Sessions 8g–8i (GC heap allocator, GC roots wiring, bench + tune) remain.
...
**v0.2 work also runs in parallel** — Phase 8 in the roadmap. **Phase 8 is now substantively complete** as of 2026-04-30. Nine feature sessions shipped: 1 (`.glb`), 2a / 2b / 2c (resumable wait + frame stack + VM nested-block parity), 3 (mouse input), 4 (save / load bottom layer), 5 (audio v2), 6 (tilemap), 7 (VM function-body wait via multi-frame fiber save). The remaining line item — **NaN-tagged 64-bit values + tracing GC** — broke out as Phase 8.5 per `docs/08-nan-tagging.md` because the migration is genuinely 9 sub-sessions; **sessions 8a–8f (structural half) shipped 2026-04-30** (8a–8e per session 8e closeout above; 8f mass-rewrote 992 `Value::Foo(...)` constructor + storage sites to the `TaggedValue` API and aliased `Value = TaggedValue`). The remaining 8f predicate-dispatch cleanup (~370 `match X.to_legacy() { LegacyValue::Foo => ... }` shim sites that still ride the renamed legacy enum) is its own follow-up session — small enough to land independently and **doesn't block 8g–8i** because every storage location and heap interior is already `TaggedValue`. **Sessions 8g–8i still pending** (GC heap allocator; roots wiring; bench + tune to 3× exit). **499 tests pass; clippy + build clean.**
