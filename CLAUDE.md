...
**Post-v0.1 the canonical plan is `docs/05-roadmap.md` Phases 13–16.** Phases 8 / 8.5 / 9 / 10 / 11 / 12 all closed; the v1.0 thesis ("ship a Vampire-Survivors-class commercial 2D game on Twe") still drives prioritization. 3D is in maintenance mode, off the v1.0 critical path. Phase 12's `examples/survive_demo` is the proof the build pipeline can deliver a Steam-class executable.
