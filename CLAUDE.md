...
- **Phase 10** (UI + game-shell primitives): **active.** Sessions 1–8 shipped 2026-05-04. Sessions 1–5 (widgets): `button`, `label`, `progress_bar`, `slider`, `checkbox`, `dropdown`, `text_input`. Session 5b: clipboard (`os.clipboard.read`/`write` via arboard, plus Ctrl+V paste in `text_input`). Session 6: `panel` + `stack` + `flex` (positioning helpers returning `{at, size}`). Session 7: `grid` + `scroll` (per-rect scroll state via `UI_STATE.scroll_y` driven by `mouse.wheel`). Session 8: explicit `pause(flag)` / `is_paused()` — the play loop skips `tick_frame` when paused. Auto-pause-on-window-blur deferred (macroquad 0.4 doesn't expose focus events; needs a winit-integration follow-on). Per-state opt-out (`pause: false` / `state foo: persistent`) remains an open syntax question per "What is open" above. The if-expression form `let x = if c: a else: b` is now parsed (closes the latent `gamepad_demo.twe:9` bug). Demos: `button_demo.twe`, `widgets_demo.twe`, `layout_demo.twe`, `pause_demo.twe`. Remaining: settings system, localization scaffolding, auto-pause-on-blur, and the exit-gate pause-menu integration.
