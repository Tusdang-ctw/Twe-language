...
- **Value representation: NaN-tagged 64-bit values** (in the bytecode VM). Follow *Crafting Interpreters* Chapter 30. **Lands in Phase 8.5** per `docs/08-nan-tagging.md` — its own multi-session sub-phase pulled out of Phase 8's close-out because the migration is genuinely 9 sessions of careful work. Sessions 8a–8e shipped 2026-04-30 (TaggedValue module, HeapBody expansion, VM/eval/stdlib storage migrations through the legacy-Value shim). **Session 8f shipped 2026-05-01 in three commits**: structural half (Value=TaggedValue alias + mass constructor migration), predicate-dispatch pass (every `match X.to_legacy()` shim site converted to `if X.is_foo() { let x = X.as_foo(); ... }`), and cleanup (deleted `LegacyValue` enum, `to_legacy`/`from_legacy` shim, `ToLegacyShim` trait, plus three round-trip tests that had nothing left to pin). The 8f exit criterion holds: zero `Value::Foo(_)` enum-variant patterns or constructors outside `tagged_value.rs`. **Session 8g shipped 2026-05-01**: new `src/heap.rs` with thread-local `Heap`, `HeapObject` GC headers (`mark`/`body_kind`/`next`), `gc_alloc`/`gc_collect` mark+sweep with recursive HeapBody scan and cycle detection, `TaggedValue` made `Copy` (no refcount; heap owns memory), `Box::into_raw` allocation replacing `Rc::into_raw`, plus 5 GC unit tests. **Session 8h shipped 2026-05-01**: GC roots wiring + safepoints. `Env::scan_roots` and `VM::scan_roots` walk every live `TaggedValue` (bindings, stack, globals, active_scene/entities, instance fields, fiber frames, CallFrame chunk constants). `gc_collect_with(scan)` closure-based collect entry, `gc_should_collect` threshold check, `mark_bc_function_constants` for chunk-pool scanning. Auto-collect fires between statements (eval) and between bytecode instructions (VM) when threshold crossed. **502 tests pass.** Session 8i (bench + tune to 3× speedup) remains.
...
**v0.2 work also runs in parallel** — Phase 8 in the roadmap. **Phase 8 is now substantively complete** as of 2026-04-30. Nine feature sessions shipped: 1 (`.glb`), 2a / 2b / 2c (resumable wait + frame stack + VM nested-block parity), 3 (mouse input), 4 (save / load bottom layer), 5 (audio v2), 6 (tilemap), 7 (VM function-body wait via multi-frame fiber save). The remaining line item — **NaN-tagged 64-bit values + tracing GC** — broke out as Phase 8.5 per `docs/08-nan-tagging.md` because the migration is genuinely 9 sub-sessions. **Sessions 8a–8h shipped** (8a–8e on 2026-04-30, 8f / 8g / 8h on 2026-05-01). 8g delivered the heap allocator + mark+sweep + Copy `TaggedValue`; 8h wires the eval and VM root sets and adds threshold-gated safepoints between statements / bytecode instructions. Auto-collection now fires under live workloads. Only **8i (bench + tune to 3× speedup)** remains. **502 tests pass; clippy clean under `-D warnings`; build zero warnings.**
