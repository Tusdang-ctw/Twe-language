...
| 6 — Particle burst | **v0.3** (waiting on event hooks) | `particles` block parses + the runtime ships on **both backends** (tree-walker eval and bytecode VM): per-particle `on_spawn(p)` / `on_update(p, dt)` fire each frame, `p.age_ratio` auto-updates, dead particles prune, the emitter despawns when empty. `spawn ExplosionBurst at e.pos` syntax also ships. The remaining gap is the global event hook `on enemy.death(e):` — that's a parser+AST+dispatch addition scheduled as a Phase 9 follow-on session. (Earlier versions of this row claimed the runtime no-ops; that was stale — the runtime landed in a prior session and went undocumented.) |
