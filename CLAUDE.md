...
**Post-v0.1 the canonical plan is `docs/05-roadmap.md` Phases 13–16 (v1.0 line) + Phases 17–26 (3D commercial arc per `docs/3d-roadmap.md`) + Phases 27–32 (post-v1.0 extensions).** Phases 1–27 are codebase-closed; the v1.0 thesis ("ship a Vampire-Survivors-class commercial 2D game on Twe") still drives prioritization. The 3D commercial roadmap shipped alongside the v1.0 line; remaining 3D work is polish (now Phase 28). Phase 12's `examples/survive_demo` is the proof the build pipeline can deliver a Steam-class executable. **Phase 27 closed 2026-05-09** per `docs/changes/2026-05-09-phase-27-closeout.md` — five sessions: `examples/platformer.twe`, `examples/tetris.twe`, `examples/cards.twe`, stdlib gap closure (added `math.mod`, `random.shuffle`, `tilemap_solid_aabb`, `tilemap_aabb_touches`), closeout. Four of eight surfaced `GAP-N` markers closed; the rest deferred with re-entry phases (sweep / key_repeat / mouse_release / hit_box). **742 tests pass.** **Phases 28–32 (post-v1.0) cover:** 3D commercial polish (28 — bloom / DoF / cascaded shadows / mipmaps / async preload), determinism layer (29 — fixed timestep, bounded GC, replay/record, sample-accurate audio; closes the 3× bytecode-VM speedup gap), WASM / web target (30), multiplayer foundation (31, gated on a netcode RFC), open-world 3D foundation (32, gated on a lock revision authorizing engine-internal worker pools). See `docs/05-roadmap.md` "Post-v1.0 — Phases 27–32" section.
