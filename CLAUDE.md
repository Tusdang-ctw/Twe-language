...
- **Value representation: NaN-tagged 64-bit values** (in the bytecode VM). Follow *Crafting Interpreters* Chapter 30. **Lands in Phase 8.5** per `docs/08-nan-tagging.md` — its own multi-session sub-phase pulled out of Phase 8's close-out because the migration is genuinely 9 sessions of careful work. Sessions 8a (TaggedValue module), 8b (HeapBody expansion), 8c (VM stack + globals on TaggedValue, full-Value shim), and 8d (Env::bindings + Instance::fields on TaggedValue) shipped 2026-04-30; sessions 8e–8i wire it through the rest of the codebase.
