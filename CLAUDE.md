...
**v0.2 work also runs in parallel** — Phase 8 in the roadmap. **Phase 8 is now substantively complete** as of 2026-04-30. Nine feature sessions shipped: 1 (`.glb`), 2a / 2b / 2c (resumable wait + frame stack + VM nested-block parity), 3 (mouse input), 4 (save / load bottom layer), 5 (audio v2), 6 (tilemap), 7 (VM function-body wait via multi-frame fiber save). The remaining line item — **NaN-tagged 64-bit values + tracing GC** — broke out as Phase 8.5 per `docs/08-nan-tagging.md` because the migration is genuinely 9 sub-sessions; **sessions 8a–8e shipped 2026-04-30** (8a TaggedValue module, 8b HeapBody expansion, 8c VM stack + globals on TaggedValue, 8d Env::bindings + Instance::fields on TaggedValue, 8e Object::fields + BcInstance::fields on TaggedValue + stdlib bootstrap shimmed). **Sessions 8f–8i still pending** (delete legacy `Value` enum — ~917 pattern-match sites; GC heap allocator; roots wiring; bench + tune to 3× exit). **499 tests pass; clippy + build clean.**
