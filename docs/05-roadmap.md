...
- `visual` block → WGSL fragment-shader compilation. Example 5 (procedural fire), finally. Subset of math + vector + color stdlib usable inside `visual` bodies. **Status (2026-05-02):** sessions 8 + 9 shipped. **Session 8** wired the parser surface (`TokenKind::Visual` + `DeclKind::Visual` + parse dispatch + LSP `SymbolKind::Visual`). **Session 9** added `crate::visual_check` — a subset typechecker that walks every `visual` block body and rejects allocating expressions (strings, lists, ranges, indexing), unbounded loops (`while` / `for`), control transfers (`break` / `continue` / `transition`), mutation (assignment), event handlers (`on …`), and any function call outside the GPU-safe whitelist (bare `smoothstep` / `mix` / `noise`; `math.<abs|sqrt|floor|ceil|min|max|sin|cos|smoothstep|mix|noise>`; `color.<named-constant>` reads — constructors like `color.from_hex` defer to runtime). Errors carry source positions and explanatory help. WGSL codegen is session 10; render integration is session 11.
