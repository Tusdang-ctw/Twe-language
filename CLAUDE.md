...
**Post-v0.1 the canonical plan is `docs/05-roadmap.md` Phases 13–16 (v1.0 line) + Phases 17–26 (3D commercial arc per `docs/3d-roadmap.md`) + Phases 27–32 (post-v1.0 extensions).** Phases 1–30 are codebase-closed; the v1.0 thesis ("ship a Vampire-Survivors-class commercial 2D game on Twe") still drives prioritization. The 3D commercial roadmap shipped alongside the v1.0 line. Phase 12's `examples/survive_demo` is the proof the build pipeline can deliver a Steam-class executable. **Phase 27 closed 2026-05-09** per `docs/changes/2026-05-09-phase-27-closeout.md` — five sessions: 2D genre examples (platformer / tetris / cards) + stdlib gap closure adding `math.mod` / `random.shuffle` / `tilemap_solid_aabb` / `tilemap_aabb_touches`. **Phase 28 closed 2026-05-09** per `docs/changes/2026-05-09-phase-28-closeout.md` — six sessions: mipmap pyramid + 16× anisotropic filtering on game textures (1), cascaded shadow maps with 3 cascades on a 2D-array depth texture (2), inline 12-tap bloom via `postfx.bloom` / `postfx.bloom_threshold` (3), vignette tint color via `postfx.vignette_color` (4), async `.glb` parse on background worker threads (5), closeout (6). DoF + view-frustum-fitted CSM + multi-tier bloom downsample + linear-space mip resample remain honest deferrals. **Phase 29 closed 2026-05-09** per `docs/changes/2026-05-09-phase-29-closeout.md` — six sessions: fixed-timestep accumulator + `time.physics_dt` (1), incremental GC sweep + `gc.budget_ms` / `gc.last_collect_ms` / `gc.bytes_alive` (2), VM immediate-int dispatch tuning via new `as_imm_int_unchecked` extractor (3), `replay.record` / `replay.play` / `replay.stop` + `tests/replay.rs` end-to-end determinism harness (4), tick-accurate audio scheduling — `sound.schedule` / `sound.now` / `sound.scheduled_count` (5), `examples/rhythm_demo.twe` + closeout (6). Bytecode VM hits the 3× speedup target on `fib_recursive` but remains 1.5–1.7× slower than tree-walker on `sum_loop` / `float_loop` — closing the dispatch-loop gap (computed-goto / direct-threading scope) is honest deferral. **Phase 30 closed 2026-05-09** per `docs/changes/2026-05-09-phase-30-closeout.md` — six sessions: `BuildTarget::Wasm32` + `build_wasm_target()` + WASM play loop (1), localStorage saves via `quad-url` (2), click-to-start overlay / AudioContext unlock (3), CSS `aspect-ratio` letterbox (4), `.github/workflows/wasm-demo.yml` CI deploy-to-Pages (5), closeout (6). Honest deferrals: end-to-end browser test in CI, `survive_beta` WASM, installed-twec WASM build path, 3D WASM. **Phase 31 closed 2026-05-10** per `docs/changes/2026-05-10-phase-31-closeout.md` — seven sessions: netcode RFC committing lockstep over UDP, max 4 peers, direct-IP only (1); `src/net.rs` UDP transport with 16-byte tagged headers, write-once per-tick local-input ring, per-peer redundant-history retransmit, non-blocking `poll()`, MSG_HELLO/INPUT/HASH/BYE framing (2); 13 `net.*` builtins — `host` / `connect` / `close` / `is_connected` / `local_peer_id` / `peer_count` / `session_ready` / `send_input` / `tick_ready` / `advance_tick` (merges peer Frames into `key`/`mouse_held`/... ambients + installs per-peer `peer[i]` list with full key-name template) / `send_state_hash` / `state_hash` / `input_delay` (3); lockstep runner + `tests/net.rs` end-to-end two-thread 30-tick exchange with rolling-hash determinism check (4); `net.hash` + `net.snapshot_json` snapshot serialization via canonical-JSON+FNV1a (5); `examples/pong_net.twe` 2-player LAN pong demo (6); closeout (7). **Phase 32 closed 2026-05-10** per `docs/changes/2026-05-10-phase-32-closeout.md` — nine sessions: CLAUDE.md "What is locked" addendum authorizing engine-internal worker pool for asset I/O / physics / spatial queries / culling / streaming (1); `src/spatial.rs` LooseGrid + median-split BVH + WorldSpatial with 7 stdlib builtins (2); `src/streaming.rs` chunk manifest + budget-bound load/unload state machine + 8 stdlib builtins (3); `src/lod.rs` LodChain + LOD_TABLE + 4 stdlib builtins (4); `src/terrain.rs` chunked heightfield + bilinear interp + central-difference normals + 7 stdlib builtins under new `terrain.*` namespace (5); `src/cull.rs` Gribb-Hartmann frustum extraction + integration with WorldSpatial / BVH + 2 stdlib builtins (6); `src/instance.rs` per-asset instance buckets + 7 stdlib builtins (7); ergonomic helpers `world.stream_radius_meters` / `entity_lod` / `world_to_lod` / `distance_xyz` (8); closeout (9). **35 new builtins across `world.*` (28) + `terrain.*` (7).** **810 tests pass; clippy clean under `-D warnings`.** Honest deferrals: wgpu render-pipeline integration (Phase 32 follow-on — data structures ship, render-side consumption is the next dev cycle), occlusion culling beyond frustum (occluder-list + GPU hierarchical-Z), `entity Tree: lod = [...]` parser sugar (v2 ergonomic pass; builtin form is fully expressive), SAH-optimal BVH build, 3D loose-grid (XZ-only currently), LOD smooth transitions, streaming asset cache, WASM port of these modules, `docs/06` §7 namespace writeup, VM mirror of new builtins, integrated 50k-prop / 500-NPC bench harness. **Phases 1–34 codebase-closed.** **Phase 33 closed 2026-05-10** per `docs/changes/2026-05-10-phase-33-closeout.md` — eleven sessions of LLM-differentiator work driving `LLMsPlan.md` from design doc to shipped: `LLMsPlan.md` strategy doc (0); `twec grammar` GBNF/JSON-Schema/EBNF export (1); `twec verify` v2 with structured `fix: { rationale, edits[{line, col, len, replace}] }` field (2); `twec stdlib --json` 235-builtin manifest with zero install-vs-manifest drift (3); `twec llm-loop` provider-trait harness with FixtureProvider + CommandProvider, per-round JSONL traces seeding fine-tune corpus (4); `twec mcp` stdio JSON-RPC 2.0 server exposing 7 tools — parse / verify / format / grammar / stdlib_list / stdlib_lookup / apply_patch (5); `twec corpus --json` + `@task / @inputs / @expected / @category / @difficulty` headers on all 40 examples (6); `twec eval` replay-based benchmark on `eval::run_with_frames` with three seed suites and three match modes (7); `twec mutate` auto-mutates `tests/programs/*.twe` to produce `(broken, verify_json, fix)` triples for fine-tune training (8); typed holes `???` — lexer + parser + AST + eval + infer + verify + printer integration; verify reports as Warning, eval errors at runtime, bytecode VM rejects with deferral message (9); closeout (10). **All Tier 1+2+3 acceptance criteria from `LLMsPlan.md` met. 912 tests pass.** **Phase 34 closed 2026-05-10** per `docs/changes/2026-05-10-phase-34-closeout.md` — six sessions of cross-platform polish: macOS focus path via `objc2` `[[NSApplication sharedApplication] isActive]` (1); Linux X11 focus path via `x11rb` parallel connection polling `_NET_ACTIVE_WINDOW` on the root window then `_NET_WM_PID` on the active window, compared to `std::process::id()` (2); Linux Wayland documented stub — focus is per-input-device and only delivered to the focused client; needs miniquad-upstream cooperation, not solvable from outside the windowing system client (3); cargo-dist matrix expansion adding `aarch64-unknown-linux-gnu` row to `release.yml` cross-compiled on x86_64 Linux runner via `gcc-aarch64-linux-gnu` linker package (4); cross-compile CI gate in `ci.yml` running `cargo check --release --target <T>` against `aarch64-unknown-linux-gnu` + `x86_64-pc-windows-gnu` on every PR — catches breakage before tag-time releases (5); closeout (6). New deps: `objc2 = "0.5"` cfg=macos + `x11rb = "0.13"` cfg=unix (already pulled transitively via arboard). `unsafe_code = "deny"` exception list extended to mention macOS `msg_send!` calls in `src/window_focus.rs`. **913 tests pass.** Honest deferrals: Wayland focus detection (upstream miniquad work — 1–2 PRs to surface focus events, then this module's Wayland branch becomes a thin reader), live macOS / X11 smoke tests (Phase 35 external-validation item — community contributor confirms behaviour on real hardware), aarch64-pc-windows-msvc binaries (no community ask yet). **Phase 35 codebase-scaffolding-closed 2026-05-10** per `docs/changes/2026-05-10-phase-35-closeout.md` — five sub-deliverables that *enable* the external-action exit criteria without doing the external action itself: API stability snapshot tooling — `twec api-snapshot` + `twec api-diff` with a deterministic FNV-1a hash over 235 builtins + 35 keywords + 6 tool versions; baseline checked in at `docs/api-snapshots/2026-05-10-baseline.json` hash `0x299e66f4068c7979` (1); cross-machine LAN multiplayer playtest harness `docs/playtest/cross-machine-lan-multiplayer.md` with two-machine recipe + pass criteria + replay-record workflow + failure-mode triage (2); 4km open-world data-side playtest stub `examples/openworld_demo.twe` exercising 1000 static props + 50 dynamic NPCs across 4km × 4km via `world.spatial_query_radius` per frame; the 50k full-scale exit-criterion target waits on wgpu render-pipeline integration follow-on (3); Steam SDK end-to-end smoke test scaffold `tests/steam_smoke.rs` with both no-feature stubs and `#[ignore]`-gated feature build tests against Spacewar AppID 480 (4); community game pipeline `docs/community-game-pipeline.md` + closeout (5). 920 tests pass (+7: 6 api_snapshot module self-tests + 1 steam_smoke no-feature test). External-action items honestly deferred to maintainer / community: itch.io paid release of `survive_beta`, Steam AppID + Spacewar sanity run on real hardware, two-machine LAN playtest, 50k-prop full-scale open-world playtest, three-cohort community pipeline outreach + ports, six-month API stability diff against the Phase 35 baseline. Remaining code-side work is Phases 36–41 (online multiplayer + rollback netcode + browser 3D + mobile + console targets + MMO; see `docs/05-roadmap.md` Round 2) plus the Phase 32 wgpu render-pipeline integration follow-on dev cycle. **Phase 36 codebase-closed 2026-05-11** per `docs/changes/2026-05-11-phase-36-closeout.md` — eight sessions: matchmaking RFC committing Steam P2P primary + STUN fallback (1); `--features steam-net` Steam P2P transport via `src/net_steam.rs` with SteamNetworkingMessages on channels 0 (reliable input) + 1 (unreliable hash); steam.rs thunks for `local_steam_id_raw` / `p2p_send` / `p2p_receive` (2); STUN binding-request + TCP rendezvous client `src/net_stun.rs` — 20-byte RFC 5389 handshake against `stun.l.google.com:19302`, three-message TCP rendezvous (`JOIN` → `PEER` / `WAIT` / `ERR`), UDP punch helper (3); lobby primitives `net.create_lobby` / `find_lobbies` / `join_lobby` / `leave_lobby` via Steam Matchmaking; mpsc+`run_callbacks` blocking wrapper for the SDK's async API; member-list-order peer-id assignment (4); reconnect handling — `disconnect_timeout` (default 5s) + per-poll `check_disconnects`; reconnect path in `handle_packet` preserves internal peer ids; `net.peer_disconnected` / `last_disconnected_peer` / `try_reconnect` / `host_migrate_if_host_lost` / `disconnect_timeout` builtins; `tests/net_reconnect.rs` end-to-end timeout + handshake validation (5); `twec build --target linux-server` partial — directory layout + launcher + bundle ship, headless play-loop runtime deferred (6); `examples/pong_net_internet.twe` covering Steam + STUN paths + reconnect (7); closeout (8). **16 new builtins added to `net.*` (32 total).** 931 tests pass (isolated-run methodology); 2 pre-existing CRLF-cascade lib failures unchanged. Honest deferrals: live Steam AppID end-to-end test (same external blocker as Phase 35), two-different-home-networks playtest, headless `twec-server` runtime, TURN relay for symmetric-NAT non-Steam path, lockstep-runner UDP/Steam transport dispatch inside `send_input`/`tick_ready` (transport selector ships; runner-side branch follows), non-Steam lobby broker (Phase 36.5 if community asks). **Phase 37 codebase-closed 2026-05-11** per `docs/changes/2026-05-11-phase-37-closeout.md` — eight sessions of rollback netcode work: rollback RFC justifying the Principle 2 carve-out via genre-dependent-concept framing + genre-to-mode table (1); `src/rollback.rs` with snapshot ring buffer (cap = `max_rewind_frames + 1`, default 8), mode flag (Lockstep | Rollback), `InputPrediction` enum (LastInputRepeat | VelocityExtrapolate), smoothing flag, stats counters; snapshot serialisation via `crate::save::encode`+`decode` for canonical-JSON round-trip determinism (2); rewind+replay scaffolding — `discard_after` / `is_replaying` / `set_replaying` / `restore_at_tick`; the actual eval-side rewind engine is honest-deferred to a follow-on session (3); predicted-input policy builtins (`rollback.set_input_prediction` / `rollback.input_prediction`) (4); rollback opt-in via `rollback.snapshot(name, value)` runtime calls — same scripts-only-no-parser-sugar shape Phase 32 used for `entity Tree: lod = [...]` (5); visual-smoothing knobs (`rollback.set_smoothing` / `rollback.smoothing`); render-side lerp math defers alongside session 3's eval-side rewind (6); `examples/fighter_demo.twe` 2-player rollback fighting demo (WASD + arrows + jump + punch; ten `rollback.snapshot` calls per tick) (7); closeout (8). **15 new builtins** (2 in `net.*` for mode switch + 13 in new `rollback.*` namespace). Combined multiplayer surface is now 45 builtins. **938 tests pass; clippy clean under `-D warnings`.** Honest deferrals: eval-side rewind engine (phase-sized refactor of `eval::run_tick` to accept overridden ambient state), `entity Fighter: rollback = true` parser sugar (v1.x ergonomic-pass item), render-side visual-smoothing math, `MSG_HELLO` mode-mismatch handshake check, `velocity-extrapolate` policy runner consumption, two-machine real-network input-feel measurement (operator action). The Principle 2 argument lives in the RFC; phrased as "netcode is a genre-dependent concept, not a single concept", the carve-out lands naturally — same family as `play.rs`/`play3d.rs` and `save_to_path`/`save.*`. **Phase 38 codebase-scaffolding-closed 2026-05-11** per `docs/changes/2026-05-11-phase-38-closeout.md` — seven sessions of browser-3D scaffolding: `BuildTarget::Wasm32_3D` variant + `dist/web-3d/` layout writer (1); wgpu-on-web audit document at `docs/changes/2026-05-11-phase-38-wgpu-on-web-audit.md` cataloguing every cfg gate that must relax for browser 3D to work + the dep-side migration list (2); new `assets.*` namespace with `is_browser` / `is_mobile` / `platform` introspection builtins (3); WebAudio scaffold reuses Phase 30's `AudioContext`-unlock path — no new code (4); `physics.body` wasm verification via rapier3d already compiling for wasm — no new code (5); `examples/crystal_hunter_web.twe` platform-aware placeholder demo (6); closeout (7). Honest deferrals: the wgpu-on-web pipeline port itself (every `cfg(not(target_arch = "wasm32"))` gate in `src/play3d.rs` relaxing to wasm-friendly equivalents, async wgpu device acquisition via `wasm_bindgen_futures`, `requestAnimationFrame` integration, asset routing via `window.fetch`, Cargo.toml move-to-unconditional). Externally gated on Firefox-stable + Safari-stable browser wgpu adoption. **Phase 39 codebase-scaffolding-closed 2026-05-11** per `docs/changes/2026-05-11-phase-39-closeout.md` — nine sessions of mobile (iOS + Android) scaffolding: `BuildTarget::IosAarch64` + `BuildTarget::AndroidAarch64` with directory-layout writers producing `Payload/<game>.app/Info.plist` + `app/src/main/AndroidManifest.xml` templates (1); new `touch.*` namespace — `is_active` / `x` / `y` / `count` / `pointer(i)` / `tap_count` wrapping `macroquad::input::touches()` (2); virtual `joystick(at:, size:, deadzone:)` widget returning `{x, y, active, magnitude}` over the (deadzone, size] band (3); `docs/mobile-signing.md` covering iOS codesign + .ipa packaging (4) and Android NDK + Gradle + .aab packaging (5) operator recipes; new `safe_area.*` namespace — `top` / `bottom` / `left` / `right` / `rect` + Rust-side `set_safe_area_insets` thread-local setter ready for the mobile-runtime platform hooks (6); `examples/survive_beta_mobile/{twe.toml, main.twe}` touch-controls reference scene composing joystick + safe-area + multi-touch visualisation + WASD desktop fallback via `assets.platform()` branching (7); App Store + Play Store submission docs in the same mobile-signing.md (8); closeout (9). **11 new builtins + the joystick widget**, 2 new namespaces, 3 new build targets bringing the total to 9 (`WindowsX86_64` / `MacOsAarch64` / `MacOsX86_64` / `LinuxX86_64` / `Wasm32` / `LinuxServer` / `Wasm32_3D` / `IosAarch64` / `AndroidAarch64`). **938 tests pass; clippy clean under `-D warnings`.** Honest deferrals: cross-compile pipeline automation (operator runs `cargo build --target aarch64-apple-ios/aarch64-linux-android` manually today), iOS / Android signing CI integration via cargo-dist, live `UIViewController.safeAreaInsetsDidChange` + `WindowInsetsCompat` platform hooks that call `set_safe_area_insets`, `touch.tap_count` play-loop hook, full `survive_beta` mobile port (multi-session content effort tracked separately), TestFlight + internal-track Play Store playtest on real hardware (operator action — exit criterion measurement). **Phase 40 codebase-scaffolding-closed 2026-05-11** per `docs/changes/2026-05-11-phase-40-closeout.md` — seven sessions of console-targets scaffolding under the NDA-bound partition model: console RFC committing platform-agnostic abstractions in public + SDK-specific code in partner private forks (1); `console.*` namespace with `controller(i)` reading Phase 9 `gamepad` ambient through Xbox-canonical naming (a/b/x/y), `controller_count`, returns "not connected" for `i > 0` until partner forks wire multi-pad (2); `console.glyph(button, style)` + `console.glyph_asset` + `console.detect_style` with xbox/playstation/switch glyph tables (3); `achievements.*` + `cloud_save.*` + `friends.*` trait stubs with Steam routes via Phase 15 + no-op fallbacks (4); `PARTNER.md` partner-fork contribution guide at repo root covering the public/private partition table + fork setup + extension points + IP attribution (5); `examples/console_demo.twe` platform-agnostic demo using only public abstractions (6); closeout (7). **13 new builtins across 4 new namespaces.** Honest deferrals: Switch/PS5/Xbox SDK ports (partner-driven; cannot ship open-source), glyph asset spritesheets (platform-owned NDA art), multi-controller enumeration past `i = 0`, platform-specific achievement/cloud-save/friends implementations, the actual exit criterion ("one first-party game on one console store" is partner-driven by design). **Phase 41 codebase-scaffolding-closed 2026-05-11** per `docs/changes/2026-05-11-phase-41-closeout.md` — nine sessions of MMO scaffolding picking **sharded zones** as the architecture: MMO architecture RFC justifying the sharded-zones pick + the Principle 2 read on `mmo.*` runtime calls vs language-level annotations (1); `mmo.replicate` / `mmo.persist` (saves to thread-local HashMap via `crate::save::encode`) / `mmo.load` stubs (2); `mmo.broadcast` + `mmo.next_event` event-queue stubs — local script observes its own broadcasts as single-tenant simulation (3); `mmo.entities_near` composing directly with Phase 32 `world.spatial_query_radius` (4); `mmo.shard_id` returns `"default"` today + `mmo.transfer_to(shard)` locally updates the active shard (5); sandboxing pre-requirements document enumerating seven blockers (gas metering / memory accounting / capability-restricted stdlib / shared-memory hardening / `unsafe` audit / deterministic+bounded stdlib / quarantine pipeline) — none met today; adversarial multi-tenant needs all seven (6); `workshop.publish` / `workshop.list_subscribed` / `workshop.install` trait stubs (7); `examples/mmo_demo/{twe.toml, main.twe}` single-player simulation exercising the full API surface (8); closeout + final round-2 doc sync (9). **11 new builtins across 2 new namespaces.** Combined post-v1.0 abstraction surface is now 45 builtins across 11 namespaces. Honest deferrals: the entire server runtime (multi-year post-v2.0); adversarial sandboxing (seven pre-requisites enumerated); replicated-entity wire format; cross-shard handoff protocol; `entity Player: replicated = true` parser sugar; server-side language-level network annotations (would re-open Principle 2 if pursued); 100-concurrent-players exit criterion; player-authored Workshop content. **938 tests pass; clippy clean under `-D warnings`** (six lints fixed during validate). **Round 2 (Phases 33–41) is now codebase-closed across the board.** Round 1 (Phases 27–32) shipped full features; Round 2's later phases (35, 38, 39, 40, 41) shipped as scaffolding-closed with closeout-enumerated deferrals. Six concrete next steps survive as follow-on items, each scoped to its predecessor's deferral list: Phase 32 wgpu render-pipeline integration, Phase 37 eval-side rewind engine, Phase 38 browser wgpu port, Phase 39 mobile-runtime safe-area hooks, Phase 40 partner SDK ports, Phase 41 server runtime. No new roadmap phases are open.
